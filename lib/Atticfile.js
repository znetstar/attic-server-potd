          ...
          let outUrl;
           if (pathname === '/potd/nasa') {
             ...
             await page.evaluate((stuff) => {
               ...
               let psa = (Array.from($('p').map(function (){ return { x: [ $(this).height(), this ] }; })).map((h) => h.x));
               let ps = new Map();
               for (let [k,v] of psa) {
                 !ps.has(k) && ps.set(k,v);
               }
             ...
             }, { jquery: require('fs').readFileSync(require('path').join(__dirname, '..', 'node_modules', 'jquery', 'dist', 'jquery.min.js')) });
